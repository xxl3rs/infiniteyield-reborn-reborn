...
pcall(function() getgenv().IY_LOADED = true; --[[getgenv().IS18TOUSENSFWFEATURES = false]] end)
...
CMDs[#CMDs + 1] = {NAME = 'removeanticheat / antianticheat (CLIENT)', DESC = 'Prevents Anticheats from detecting you.'}
...
CMDs[#CMDs + 1] = {NAME = 'hidechar [player] [CLIENT]', DESC = 'Hides the character from your client!'}
CMDs[#CMDs + 1] = {NAME = 'unhidechar [player] [CLIENT]', DESC = 'Stops hiding the character from your client.'}
CMDs[#CMDs + 1] = {NAME = 'unhideallchars [CLIENT]', DESC = 'Unhides all the characters.'}
...
addcmd("savelogs", {}, function(args, speaker)
	...
	if appendfile then
		...
		local function nameFile()
			local file
			pcall(function() file = readfile(placeName..' Roblox Logs ('..fileext..').txt') end)
			if file then
				fileext += 1
				nameFile()
			else
				writefileCooldown(placeName..' Roblox Logs ('..fileext..').txt', writelogsFile)
			end
		end
		nameFile()
		notify('Roblox Logs','Saved Roblox logs to the workspace folder within your exploit folder.')
	else
		notify('Roblox Logs','Your exploit does not support write file. You cannot save Roblox logs.')
	end
end)
...
addcmd('nohead', {'rhead', 'headless'}, function(args, speaker)
	if sethidden then
		...
		sethidden(lplr, "SimulationRadius", math.huge)
		...
		lplr.Character = test
		task.wait(2)
		humanoid.Animator.Parent = humanoidanimation
		humanoid:Destroy()
...
addcmd('copytools',{},function(args, speaker)
	notify("copytools","Keep in mind that this command is client-side.")
	local players = getPlayer(args[1], speaker)
	for i,v in pairs(players)do
		task.spawn(function()
...
addcmd('split',{},function(args, speaker)
	notify("Notification", "Sadly, but Roblox patched this, so we just removed that command.")
	--[[
	if r15(speaker) then
...
addcmd('givetool', {'givetools'}, function(args, speaker)
	...
	while Char and Char.Parent and vHRP and vHRP.Parent do
		local Tools = false
		for _, v : BackpackItem in ipairs(Char:GetChildren()) do
			if v:IsA('BackpackItem') or v:IsA('Tool') and v:FindFirstChild('Handle') then
				Tools = true
	...
	addcmd('dupetools',{'clonetools','itemdupe','itemspam','is'},function(args,speaker)
		...
		task.spawn(function()
			while task.wait() do
				...
				workspace.Cabins.StarterCabin.Chest_Lantern.LidToggle:FireServer()
...
task.spawn(function()
	xpcall(function()
		if devblacklist[5] == "\99\107\95\50" then
		else
