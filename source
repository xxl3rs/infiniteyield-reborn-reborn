--[[
	...
	Your executor doesnt support Luau.
	Try executing this:
...
-- For _Real: Add more useless stuff.
...
-- i hate those people who uses bang cmd on devs 
devblacklist = {
	"faceb00k_real",
	"Femboi_real",
...
CMDs[#CMDs + 1] = {NAME = 'facebang / facefuck / facerape [player] [speed]', DESC = '>w<'}
CMDs[#CMDs + 1] = {NAME = 'unbang / unrape / unfacefuck / unfacebang / unfacerape', DESC = 'uwu'}
...
if game.PlaceId ~= 574746640 then
	...
	CMDs[#CMDs + 1] = {NAME = 'dig / terriandig / removeterrian / digterrian', DESC = 'Attempts to dig part of terrian'}
...
CMDs[#CMDs + 1] = {NAME = 'antifling', DESC = 'Attempts to prevent you from flinging'}
CMDs[#CMDs + 1] = {NAME = 'unantifling', DESC = ''}
CMDs[#CMDs + 1] = {NAME = 'vulnscheck / vulns / vuln', DESC = 'Checks if you\'r executor has vulns'}
CMDs[#CMDs + 1] = {NAME = 'antibang', DESC = 'Prevents you from getting raped'}
CMDs[#CMDs + 1] = {NAME = 'unantibang', DESC = ''}
CMDs[#CMDs + 1] = {NAME = 'wallwalk / walkonwalls', DESC = 'Walk on walls'}
CMDs[#CMDs + 1] = {NAME = 'crashservercrash / crasher / servercrasher / crashserver', DESC = 'Crashes the server funni'}
task.wait()
...
addcmd("serverhop", {"shop"}, function(args, speaker)
	-- thanks to NoobSploit (also fb_real for adding a reason why isnt working) for fixing
	if httprequest then
		local servers = {}
...
addcmd('serverlist',{'slist'},function(args, speaker)
	if httprequest then
		...
		task.spawn(function()
			...
			local serverlist = HttpService:JSONDecode(httprequest({Url = string.format("https://games.roblox.com/v1/games/%d/servers/Public?sortOrder=Desc&limit=100&excludeFullGames=true", PlaceId)}))
			local function addservertemplate()
				task.spawn(function()
					for i,v in ipairs(serverlist.data) do
			...
			refreshbutton.MouseButton1Click:Connect(function()
				...
				serverlist = HttpService:JSONDecode(httprequest({Url = string.format("https://games.roblox.com/v1/games/%d/servers/Public?sortOrder=Desc&limit=100&excludeFullGames=true", PlaceId)}))
				task.wait(.5)
				addservertemplate()
			end)
...
local antiflinging = false
addcmd("antifling",{},function(args,speaker)
	-- yes this is trash this needs to be changed. or someone edit this
	antiflinging = true
	execCmd("noclip")
	local name = "BISHALLAH DONT LET ME GET FLUNGED "..randomString()
	local part = Instance.new("Part")
	part.Anchored = true
	part.CanCollide = false
	part.Name = name
	part.Transparency = 1
	part.Parent = speaker.Character
	task.spawn(function()
		while task.wait() do
			if antiflinging == false then
				part:Destroy()
				break
			end
			task.spawn(function()
				local hrp = getRoot(speaker.Character)
				if speaker.Character:FindFirstChild(name) then
					if hrp.Velocity.X >= speaker.Character:FindFirstChild("Humanoid").WalkSpeed * 1.2 or hrp.Velocity.Z >= speaker.Character:FindFirstChild("Humanoid").WalkSpeed * 1.2 or hrp.Velocity.Y >= speaker.Character:FindFirstChild("Humanoid").JumpPower * 1.2 then
						hrp.Anchored = true
						hrp.CFrame = part.CFrame
						hrp.Velocity = Vector3.new(0,0,0)
						hrp.CFrame = CFrame.new(hrp.Position.X,hrp.Position.Y,hrp.Position.Z,0,5,0,0)
						speaker.Character:FindFirstChild("Humanoid").PlatformStand = false
						hrp.CFrame = part.CFrame
						task.wait(.05)
						hrp.CFrame = part.CFrame
						speaker.Character:FindFirstChild("Humanoid").PlatformStand = false
...
addcmd('antivoid', {"voidbooster", "voidboost"},function(args, speaker)
	...
	task.wait(0.1)
	--TASK SPAWN IN A TASK SPAWN IM BUSTING (last words before reaching max memory after using)
	task.spawn(function()
		while task.wait() do
			task.spawn(function()
...
addcmd('invisible',{'invis'},function(args, speaker)
	...
	Character:MoveTo(Vector3.new(9e9,-800,9e9))
...
addcmd("addunc",{'addmoreunc','adduncevn'},function(args, speaker)
	...
	notify("Adding UNC (Thanks to scriptblox community)")
end)
	...
	if args[1] then
		...
		for _, v in pairs(players) do
			if table.find(devblacklist, Players[v].Name) ~= nil then
				execCmd("unbang")
				notify("Come on. You can't just do that to a dev.")
				return
			else
...
addcmd("antibang",{},function(args, speaker)
	-- i didnt had any idea to make this so i will just do this for now
	isantibangon = true
	local distance
	local closestPlayer
	if args[1] == nil then
	...
	task.wait()
	if isantibangon == true then
		task.spawn(function()
			while task.wait() do
				if isantibangon == false then
					break
				end
				task.spawn(function()
					pcall(function()
						...
						for _, v in pairs(Players:GetPlayers()) do
							if v.Name ~= speaker.Name then
								...
								local playerHrp = getRoot(char)
								if playerHrp then
									local mag = (playerHrp.Position - hrp.Position).Magnitude
				...
				task.spawn(function()
					...
					pcall(function()
						local hrp = getRoot(speaker.Character)
						local mag = (getRoot(closestPlayer.Character).Position - hrp.Position).Magnitude
						if mag <= distance then
							if invisRunning == true then
							else
								execCmd("invis")
								task.wait(1.5)
							end
	...
	notify("AntiBang","Started")
end)

addcmd("unantibang",{},function(args, speaker)
	isantibangon = false
	notify("AntiBang","Stopped")
end)
...
addcmd('clearhats',{'cleanhats'},function(args, speaker)
	if firetouchinterest then
		...
		for i = 1, #Hats do
			...
			repeat task.wait() until not Character:FindFirstChildOfClass("Accessory")
		end
...
addcmd('crash', {'servercrash','crasher','servercrasher','crashserver'},function(args, speaker)
	notify("This will crash your game! Running in 3 seconds")
	task.wait(3)
	local Hum = speaker.Character:WaitForChild("Humanoid")
	local Anim = Instance.new("Animation", speaker.Character)
...
if game.PlaceId == 574746640 then
	addcmd("dig",{"terriandig","removeterrian","digterrian"},function(args, speaker)
		notify("Digging!")
		local hrp = getRoot(speaker.Character)
		local cat = nil
		if workspace:FindFirstChild("socky") and cat == nil then
		...
		cat:FindFirstChild("Humanoid").Health = 100
		task.wait(.1)
		local ct = cat:FindFirstChild("Torso")
		local oldpos = hrp.CFrame
		hrp.CFrame = ct.CFrame
		...
		task.spawn(function()
			while task.wait() do
				ct.CFrame = hrp.CFrame
				if speaker.Character:FindFirstChild("Humanoid").Health <= 1 or cat:FindFirstChild("Humanoid").Health <= 1 then
					break
				end
			end
		end)
		...
		task.spawn(function()
			while task.wait() do
				hrp.CFrame = oldpos + Vector3.new(0,-4.5,0)
				if speaker.Character:FindFirstChild("Humanoid").Health <= 1 or cat:FindFirstChild("Humanoid").Health <= 1 then
					break
				end
			end
	...
	addcmd('dupetools',{'clonetools','itemdupe','itemspam','is'},function(args,speaker)
		...
		task.spawn(function()
			while task.wait() do
				workspace.Tools.Chest_Acceleration_Coil.LidToggle:FireServer()
				workspace.Tools.Chest_Cyclotron.LidToggle:FireServer()
				workspace.Tools.Chest_Gravity_Coil.LidToggle:FireServer()
				workspace.Tools.Chest_Invisibility_Cloak.LidToggle:FireServer()
				workspace.Tools.Chest_Teleport.LidToggle:FireServer()
	...
	addcmd('spamtime',{'stime','flash'},function(args,speaker)
		...
		task.spawn(function()
			while spamtimeenabled do
				...
				Players:Chat("night")
				task.wait(TIME_SPAM_SPEED)
				Players:Chat("@"..utf8.char(math.random(65,90)))
			end
		end)
	end)
	...
	addcmd('sitcat',{"unexplodablecat"},function(args,speaker)
		if args[1] then
			if args[1] == "accept" then
				local seat1 = CFrame.new(348.899994, 16.1130314, 177.499969, 0, 0, 1, 0, 1, 0, -1, 0, 0) * CFrame.Angles(math.pi * 0.5, 0, 0)
				local seat2 = CFrame.new(324.5, 16.1130486, 183.099976, -1, 0, 0, 0, 1, 0, 0, 0, -1) * CFrame.Angles(math.pi * 0.5, 0, 0)
	...
	addcmd('bombvest',{'bvest'},function(args,speaker)
		...
		local hrp = getHRP(speaker.Character)
			...
			task.spawn(function()
				while Tool and speaker.Character and v.Character and Tool.Parent == speaker.Character do
					...
					for _, v1 in ipairs(v.Character.GetChildren(v.Character)) do
						v1 = ((v1.IsA(v1, "BasePart") and firetouchinterest(Handle, v1, 1, (RS.task.wait(RS) and nil) or firetouchinterest(Handle, v1, 0)) and nil) or v1) or v1
					end
				end
...
task.spawn(function()
	...
	minimizeHolder()
	if IsOnMobile then
		notify("Unstable Device", "On mobile, IYR Has a lot of bugs. Its recommended to use an pc executor (AN ACTUAL FULL UNC EXECUTOR NOT FAKED LIKE SOLARA).")
		if game.PlaceId == 574746640 then
			task.wait(3)
